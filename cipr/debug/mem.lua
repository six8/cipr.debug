...
--
-- 
-- Adapted from Alexandra Barros - 2006.03.15 (http://lua-users.org/lists/lua-l/2006-07/binyu6K6iWzEF.bin)
-------------------------------------------------------------------------------
...
--      key.
local function edge(env, from, to, how, value)
    ...
    if to and (t~="boolean") and (t~="number") and (t~="new") then
        ...
        env.funcs.edge(from, to, how, value)
...
function _traverse.table(env, obj)
...
function _traverse.string(env, obj)
    local f = env.funcs.string
    if f then f(obj) end
    
end

function _traverse.userdata(env, obj)
...
_traverse['function'] = function(env, obj)
...
function _traverse.thread(env, t)
...
local function traverse(funcs, ignoreobjs)
...
function _M.findallpaths(obj)
    
    local comefrom = {}
    local f = function(from, to, how, value)
        if not comefrom[to] then comefrom[to] = {} end
        tinsert(comefrom[to], 1, {f = from, h = how, v=value})     
    end
    
    traverse({edge=f}, {comefrom, f})
    
    local function printpath(to, depth)        
        if not to or to == _G then
            print("-- top --", to)
            return
        elseif comefrom[to].visited then
            print("-- visited", to, comefrom[to])
            return
        end
                
        comefrom[to].visited = true
        for i=1, #comefrom[to] do
            local p = sformat('%s from %s `%s`', tostring(comefrom[to][i].f), tostring(comefrom[to][i].h), tostring(comefrom[to][i].v))
            if depth > 0 then
                p = string.rep(' ', depth * 2) .. '\\_ ' .. p
            end
            print(p)

            printpath(comefrom[to][i].f, depth + 1)
        end         
    end
...
--[[
Call function `func` on each item in table `root`.

func should expect value and depth.
]]--
function _M.traverse(root, func)
    local seen = {}
    local traverse_table
    traverse_table = function(t, depth)
        if seen[t] then return end
        func(t, depth)
        seen[t] = true
        for k,v in pairs(t) do
            if type(v) == 'table' then
                traverse_table(v, depth + 1)
            elseif type(v) == 'userdata' then
                func(v, depth)
            end
        end
    end
    traverse_table(root, 0)
end
